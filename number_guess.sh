...
if [[ -z $1 ]]
 ...
 #echo "$SECRET_NUMBER"
  ...
  if [[ $INPUT =~ ^[0-9]+$ ]]
    ...
    # input is the number
      case $INPUT in
        $SECRET_NUMBER) ENDGAME "$GUESS" "$SECRET_NUMBER";;
        *) CONTGAME "$INPUT";;
      esac
    else
    # other input
...
ASK_USERNAME
  ...
  then 
  #user already exist
      ID_USER=$($PSQL "SELECT id_user FROM users WHERE username = '$NAME'")
...
if [[  $ID_USER ]]
  ...
  INSERT_GAME=$($PSQL "INSERT INTO games(nbre_guess, id_user) VALUES ('$1', '$ID_USER')")
  #echo $INSERT_GAME
  echo -e "\nYou guessed it in $1 tries. The secret number was $2. Nice job!"
  else 
  echo ERROR!
fi
